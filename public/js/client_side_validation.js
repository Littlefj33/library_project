(function () {
  ...
  const phoneNumberError = document.querySelector(
    "#phoneNumberInput + span.error"
  );
  const confirmPasswordError = document.querySelector(
    "#confirmPasswordInput + span.error"
  );
  ...
  const attendingFeeError = document.getElementById(
    "#attending_fee + span.error"
  );
  ...
  if (createEventForm) {
    ...
    dateTime.addEventListener("input", () =>
      validateInput(dateTime, dateTimeError)
    );
    ...
    attendingFee.addEventListener("input", () =>
      validateInput(attendingFee, attendingFeeError)
    );
    capacity.addEventListener("input", () =>
      validateInput(capacity, capacityError)
    );
    ageLimit.addEventListener("input", () =>
      validateInput(ageLimit, ageLimitError)
    );
    ...
    createEventForm.addEventListener("submit", (event) => {
      ...
      const isAttendingFeeValid = validateInput(
        attendingFee,
        attendingFeeError
      );
        ...
        switch (
          ...
          case "title":
            ...
            if (
              ...
              inputElement.value.length > 25
            ) {
              errorElement.textContent =
                "Name must be 2-25 characters long and cannot contain numbers or special characters.";
              errorElement.className = "error active";
              return false;
            }
            break;
          ...
          case "date_time":
            ...
            if (
              new Date(inputElement.value).getFullYear() >
              new Date().getFullYear() + 5
            ) {
              errorElement.textContent =
                "Please select a date within the next 5 years.";
              errorElement.className = "error active";
              return false;
            }
            break;
          ...
          case "state":
            inputElement.value = inputElement.value.trim();
            if (
              !/^[A-Z]{2}$/.test(inputElement.value.toUpperCase()) ||
              !validStates.includes(inputElement.value.toUpperCase())
            ) {
              errorElement.textContent =
                "Please enter a valid 2-letter state code (e.g., NY).";
              errorElement.className = "error active";
              return false;
            }
            break;
