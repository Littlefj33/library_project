...
export async function usersBlogsEventsSeed() {
  // add the parameter bookIds if want to populate the bookIds
  ...
  try {
    const seedDataRaw1 = JSON.parse(
      await readFile("./seeds/users.json", "utf8")
    );
    ...
    const seedDataRaw2 = JSON.parse(
      await readFile("./seeds/blogs.json", "utf8")
    );
    ...
    const seedDataRaw3 = JSON.parse(
      await readFile("./seeds/events.json", "utf8")
    );
...
export async function seedData() {
  await usersBlogsEventsSeed(booksId)
    .catch(console.error)
    .then(() => {
