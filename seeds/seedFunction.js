...
import { users, books, blogs, events } from "../config/mongoCollections.js";
...
export async function seedData() {
  ...
  try {
    ...
    let book1_Id = (
      await dbTool(bookCollection, "title", "Prime Suspect 3", { _id: 1 })
    )[0]["_id"];
    let book2_Id = (
      await dbTool(bookCollection, "title", "White Hunter, Black Heart", {
        _id: 1,
      })
    )[0]["_id"];
    let book3_Id = (
      await dbTool(bookCollection, "title", "Bambou", { _id: 1 })
    )[0]["_id"];
    let book4_Id = (
      await dbTool(
        bookCollection,
    ...
    let book5_Id = (
      await dbTool(bookCollection, "title", "Scissere", { _id: 1 })
    )[0]["_id"];
    ...
    const event1_Id = (
      await dbTool(eventCollection, "title", "Title 1 Here", { _id: 1 })
    )[0]["_id"];
    const event2_Id = (
      await dbTool(eventCollection, "title", "Title 2 Here", { _id: 1 })
    )[0]["_id"];
    ...
    const blog1_Id = (
      await dbTool(blogCollection, "title", "Title 1 Here", { _id: 1 })
    )[0]["_id"];
    const blog2_Id = (
      await dbTool(blogCollection, "title", "Title 2 Here", { _id: 1 })
    )[0]["_id"];

    await blogFunc.addComment(
      blog1_Id.toString(),
      emailAddress2,
      "Comment 1 Here"
    );
